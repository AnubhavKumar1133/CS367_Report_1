...
class AStarSearchAgent:
    ...
    def search(self):
        ...
        priority_queue = [(self.problem.marbles_left_heuristic(initial_state), 0, initial_state_tuple)]
            ...
            for new_state in new_states:
                ...
                for i, (__, cost, state) in enumerate(priority_queue):
                    if state == new_state_tuple:
                        found = True
                        if new_cost < cost:
                            heu = self.problem.marbles_left_heuristic(new_state)
                            priority_queue[i] = (heu+new_cost, new_cost, new_state_tuple)
                            heapq.heapify(priority_queue)
                        break
                
                if not found:
                    heapq.heappush(priority_queue, (self.problem.marbles_left_heuristic(new_state), new_cost, new_state_tuple))
                    states_explored += 1
